## 「Rubyによるデザインパターン」

### 動的動機付け
* 渡されてくるオブジェクトが特定のクラスに属していないことを言語でチェックしないことを意味する
* テンプレートメソッドで言えば、クライアントが呼びだそうとしているメソッドをオブジェクトが実装していることが大事
* 型に対するこのアプローチはダックタイピングと呼ばれる

### ダックタイピング
* 「もしアヒルのように見えて、アヒルのように鳴くのなら、それはアヒルである」（"If it walks like a duck and quacks like a duck, it must be a duck."）

### GoF
* 変わるものを変わらないものから分離する
* インターフェイスに対してプログラムし、実装に対して行わない
* 継承より集約
* 委譲、委譲、委譲
* 必要になるまで作るな

### Template Method
* アルゴリズムに多様性を持たせたい場合に便利
* 変わるものと変わらないものを分離することができる
* 基底クラスに不変の部分を記述し、変わる部分はサブクラスに定義するメソッドにカプセル化する
* 基底クラスはメソッドを未定義にしておくことができる
* 未定義にする代わりに、基底クラスで標準実装を提供し、必要があればサブクラスでオーバーライドさせることもできる

### Strategy Pattern
* Template Methodパターンは継承の上に成り立っているという点に欠点がある
* 継承に基づいた設計はいくつかの深刻な不利益をもたらす
  * サブクラスはスーパークラスに依存
  * 実行時の柔軟性にも制限がつく
  * 特定のバリエーションのアルゴリズムをひとたび選択すると、方針を変えることが難しくなる
* クラスからストラテジを取り出すことで関心の分離をうまく達成することができる
* 核となるアイディアは、同じ目的を持った一群のオブジェクト（ストラテジ）を定義すること
* それぞれのストラテジオブジェクトは同じ仕事をこなすだけでなく、そのすべてが正確に同じインターフェイスを提供する
* Strategyパターンは委譲と集約に基いているので、継承に基づく方法よりも、実行時にストラテジを切り替えるのが楽になる
* StrategyパターンはTemplate Methodパターンと同様、バリエーションの選択に対する決定を1ヶ所かせいぜい数カ所に集中できる。Template Methodパターンでは具象サブクラスを選択するときに決定、Strategyパターンでは実行時にストラテジを選択することで決定する。